...
class HMMTrainer(object):
    def __init__(self,model_name='GMMHMM',n_components=1, n_mix=32 ,cov_type='spherical',n_iter=300):
...
if __name__ == '__main__':
    ...
    print(cm)
    acc = accuracy_score(test_label_numeric, predict_labels)
    toc_end = time.time()
    ...
    ranked_scores = np.flipud(np.argsort(all_scores[:, 0]))
    ...
    test_labels_top5p  = test_label_numeric[top5p]
    test_labels_top10p = test_label_numeric[top10p]
    test_labels_top15p = test_label_numeric[top15p]
    test_labels_top20p = test_label_numeric[top20p]
    test_labels_top25p = test_label_numeric[top25p]
    test_labels_top30p = test_label_numeric[top30p]

    predicted_labels_top5p  = predict_labels[top5p]
    predicted_labels_top10p = predict_labels[top10p]
    predicted_labels_top15p = predict_labels[top15p]
    predicted_labels_top20p = predict_labels[top20p]
    predicted_labels_top25p = predict_labels[top25p]
    predicted_labels_top30p = predict_labels[top30p]
